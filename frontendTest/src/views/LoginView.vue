...
const login = async () => {
  ...
  try {
    const response = await fetch('/api/login', {
      ...
      credentials: 'include' // <--- TAMBAHKAN INI! Penting untuk mengirim cookie sesi login
    });
    ...
    message.value = err.message || 'Terjadi kesalahan saat login.';
    success.value = false;
  } finally {
    loading.value = false;
  }
...
<style scoped>
/* Gunakan style yang sama dengan RegisterView.vue */
.auth-container {
  display: flex;
...
.checkbox-group label {
    ...
    font-weight: normal; /* Untuk label checkbox, bisa normal */
}
